...
function displayResults() {
    const consultantName =
        consultantNameInput.value || 'Consultante Anónimo';
    ...
    selectedRepresentatives.forEach(rep => {
        ...
        if (groupedByChakra[chakra]) {
            groupedByChakra[chakra].representatives.push(rep);
        }
    });
    ...
    selectedStellarCodes.forEach(code => {
        ...
        if (groupedByChakra[chakra]) {
            groupedByChakra[chakra].stellarCodes.push(code);
        }
    });
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        if (
            chakraGroup.representatives.length > 0 ||
            chakraGroup.stellarCodes.length > 0
        ) {
            const chakraName =
                chakrasData[i]?.name || 'Desconocido';

            const chakraColor = getChakraColor(i);

            output += `
                <span style="color: ${chakraColor};">
                    <strong>Chakra ${i} (${chakraName}):</strong>
                </span>
                <br><br>
            `;
            ...
            if (chakraGroup.representatives.length > 0) {
                output += `
                    <span style="color: #FF8C00;">
                        <strong>Representantes Asociados:</strong>
                    </span>
                    <br>
                `;
                ...
                chakraGroup.representatives.forEach(rep => {
                    const repData =
                        currentRepresentativesData[rep.number] ||
                        'Desconocido';
            ...
            if (chakraGroup.stellarCodes.length > 0) {
                output += `
                    <span style="color: #0000FF;">
                        <strong>Códigos Estelares:</strong>
                    </span>
                    <br>
                `;
                ...
                chakraGroup.stellarCodes.forEach(code => {
                    const codeData =
                        currentStellarCodesData[code.number] || null;

                    if (codeData) {
                        output += `
                            <span style="color: #333333;">
                                - <strong>${codeData.title}:</strong>
                            </span>
                            <br>
                        `;

                        output += `
                            <em>"${codeData.phrase}"</em>
                            <br>
                        `;

                        output += `
                            <span style="color: #008000;">
                                <strong>Ante Frase:</strong>
                            </span>
                            ${codeData.act}
                            <br><br>
                        `;
                        ...
                        output += `
                            - Código no encontrado para el número
                            ${code.number}
                            <br><br>
                        `;
                    }
...
addRepresentativeButton.addEventListener('click', () => {
    if (!isShuffled) {
        resultsDisplay.innerHTML = `
            <p style="text-align: center; color: red; font-weight: bold;">
                🛑 ERROR: Debes activar el Nuevo Orden Divino antes de cargar representantes.
            </p>
        `;
        return;
    }
    ...
    if (!repNumber) {
        alert('Ingresa un número de representante.');
        return;
    }

    if (!originalRepresentativesData[repNumber]) {
        alert('Número de representante no válido.');
        return;
    }

    if (!chakrasData[repChakra]) {
        alert('Chakra no válido (1-7).');
        return;
    }

    if (selectedRepresentatives.length >= maxSelections) {
        alert('Límite alcanzado.');
        return;
    }

    selectedRepresentatives.push({
        number: repNumber,
        chakra: repChakra
    });
...
addStellarCodeButton.addEventListener('click', () => {
    ...
    if (!codeNumber) {
        alert('Ingresa un número de código.');
        return;
    }

    if (!originalStellarCodesData[codeNumber]) {
        alert('Código no válido.');
        return;
    }

    if (!chakrasData[codeChakra]) {
        alert('Chakra no válido (1-7).');
        return;
    }

    if (selectedStellarCodes.length >= maxSelections) {
        alert('Límite alcanzado.');
        return;
    }

    selectedStellarCodes.push({
        number: codeNumber,
        chakra: codeChakra
    });
...
readButton.addEventListener('click', () => {
    if (!isShuffled) {
        resultsDisplay.innerHTML = `
            <p style="text-align: center; color: red; font-weight: bold;">
                🛑 ERROR: Debes activar el Nuevo Orden Divino antes de realizar una Lectura.
            </p>
        `;
        return;
    }
...
shuffleButton.addEventListener('click', () => {
    currentRepresentativesData =
        shuffleData(originalRepresentativesData);

    currentStellarCodesData =
        shuffleData(originalStellarCodesData);
    ...
    resultsDisplay.innerHTML = `
        <p style="text-align: center; color: #8A2BE2; font-weight: bold;">
            ✅ Nuevo Orden Divino Activado. ¡Comienza!
        </p>
    `;
...
clearButton.addEventListener('click', () => {
    ...
    resultsDisplay.innerHTML = `
        <p style="text-align: center;">
            Agrega tus selecciones y haz clic en "Lectura".
        </p>
    `;
...
exportPDFButton.addEventListener('click', () => {
    ...
    /*
     * LOGO OFICIAL PARA EL PDF
     * Se utiliza directamente desde Wix.
     */
    const logoURL =
        "https://static.wixstatic.com/media/0ae964_84484a238d7c433b8dfbd20f84054db3~mv2.png/v1/fill/w_417,h_437,al_c,q_85,usm_0.66_1.00_0.01,enc_avif,quality_auto/LOGO%20DE%20REDES.png";
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <title>
                Lectura Sistema Divino - ${consultantName}
            </title>
        ...
        <body>
            ...
            <div
                class="footer"
                style="font-size:13px; text-align:center; font-weight:normal;"
            >

                Este sistema es desarrollado por
                El.Alquimista.Arg® 🔮

                <br>

                <a
                    href="https://www.alquimiasdelavida.com/una-energia-que-limpia"
                    target="_blank"
                    style="color:#0000EE; text-decoration:underline; font-weight:normal;"
                >
                ...
                <a
                    href="https://www.alquimiasdelavida.com"
                    target="_blank"
                    style="color:#0000EE; text-decoration:underline; font-weight:normal;"
                >
